...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of jamstack-scripts'
  )
  .option('--use-npm')
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/sw-yx/create-jamstack-app/issues/new'
      )}`
    );
    console.log();
  })
...
function createApp(name, verbose, version, useNpm, template) {
  ...
  console.log(`Creating a new JAMstack app in ${chalk.green(root)}.`);
  console.log();
  ...
  if (!semver.satisfies(process.version, '>=6.0.0')) {
    ...
    // Fall back to latest supported jamstack-scripts on Node 4
    version = 'jamstack-scripts@0.9.x';
  }
  ...
  if (!useYarn) {
    ...
    if (!npmInfo.hasMinNpm) {
      ...
      // Fall back to latest supported jamstack-scripts for npm 3
      version = 'jamstack-scripts@0.9.x';
    }
  }
...
function run(
  ...
  getPackageName(packageToInstall)
    ...
    .then(packageName => {
      ...
      if (version === 'jamstack-scripts@0.9.x') {
        console.log(
          chalk.yellow(
...
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = 'jamstack-scripts';
